...
-->
                            ...
                            <cei:text xmlns:cei="http://www.monasterium.net/NS/cei" type="charter">
                                ...
                                <cei:front>
                                    <cei:sourceDesc>
                                        ...
                                        <cei:sourceDescRegest>Gabriele Bartz (Kunsthistorische Beschreibung), Markus Gneiß (diplomatische Beschreibung) im Rahmen des FWF Projekts "Illuminierte Urkunden"</cei:sourceDescRegest>
                                    </cei:sourceDesc>
                                </cei:front>
                                <cei:body>
                                    <xsl:copy-of select="$id/cei:idno"/>
                                    <cei:chDesc>
                                        <cei:class/><!-- Hier käme auf eigentlich die Urkundenart hinein -->
                                        <cei:abstract>
                                        ...
                                        <cei:witnessOrig>
                                            <cei:traditioForm>orig.</cei:traditioForm><!-- FixMe: es gibt auch kopiale Überlieferungen, die vermutlich am Einleitungswort "kopial" in der Archiv-Spalte erkennbar sind. -->
                                            <xsl:choose>
                                            ...
                                            <cei:archIdentifier>
                                                <cei:placeName>
                                                <xsl:value-of select="t:cell[6]/t:*[@rend='Archivort']"></xsl:value-of>
                                                </cei:placeName>
                                                <cei:arch><xsl:value-of select="t:cell[6]/t:*[@rend='Archivname']"/></cei:arch>
                                            </cei:archIdentifier>
                                            <cei:physicalDesc>
                                                <cei:decoDesc>
    ...
    <xsl:template match="t:*[@rend='Regest']">
        <xsl:if test="preceding-sibling::t:*[@rend='Regest']"><lb/></xsl:if>
        <xsl:apply-templates/>
    </xsl:template>
    <!-- 
    ...
    <xsl:template match="t:cell[5]" priority="2">
        ...
        <xsl:if test="t:p[@rend='NIVEAU']"><cei:p><xsl:apply-templates select="t:p[@rend='NIVEAU']"/></cei:p></xsl:if>
    </xsl:template>
    <xsl:template match="t:*[@rend='NIVEAU']" priority="1">
      <xsl:variable name="stringlist" select="tokenize(.,':')" />
      
            
    <xsl:if test="preceding-sibling::t:*[@rend='NIVEAU']"><xsl:text> - </xsl:text></xsl:if>
        <cei:index>
    ...
    <xsl:template match="t:ref">
        <cei:ref>
            <xsl:attribute name="target"><xsl:value-of select="normalize-space(.)"/>
            </xsl:attribute><xsl:apply-templates/></cei:ref>
    </xsl:template>
